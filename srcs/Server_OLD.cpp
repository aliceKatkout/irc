...
/*   Updated: 2024/01/24 14:03:18 by mrabourd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	while (1){
// 		if (poll(_pollfds.begin().base(), _pollfds.size(), -1) )
// 	}
// }

int Server::find_user_fd(int fd)
{
	std::map<int, User*>::iterator it = this->_connectedUsers.begin();
	std::map<int, User*>::iterator ite = this->_connectedUsers.end();
	...
	while (true) {
		...
		for (int i = 0; i < numEvents ; ++i) {
			...
			if (events[i].data.fd == this->_server_fd){
				...
				newUser->getFd();
				this->_connectedUsers.insert(std::pair<int, User *>(clientFd, newUser));

				std::cout << "this->_connectedUsers.size(): " << this->_connectedUsers.size() << std::endl;
